...
export default function AdminLiveFeed() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('patrol_update', handleStorageChange);

    // Poll every 2 seconds as a foolproof fallback for cross-tab sync
    const interval = setInterval(loadAlerts, 2000);

    return () => {
      window.removeEventListener('storage', handleStorageChange);
      window.removeEventListener('patrol_update', handleStorageChange);
