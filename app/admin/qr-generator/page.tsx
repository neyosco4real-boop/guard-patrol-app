...
export default function QRGeneratorPage() {
  ...
  const handleCreateCheckpoint = async (e: React.FormEvent) => {
    ...
    // Assign QR code directly to this specific checkpoint
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 md:p-10">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-800">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white">Checkpoint & QR Code Generator</h1>
            <p className="text-sm text-slate-400 mt-1">Create individual checkpoints and assign their dedicated QR codes.</p>
          </div>
          <a 
            href="/admin/dashboard" 
            className="bg-slate-900 hover:bg-slate-800 text-slate-300 text-xs font-semibold px-4 py-2 rounded-lg border border-slate-700 transition-colors"
          >
            ← Back to Dashboard
          </a>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 mb-8 shadow-xl">
          ...
          <form onSubmit={handleCreateCheckpoint} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <input
              type="text"
              placeholder="Checkpoint Name (e.g. Back Gate, Vault Room)..."
            ...
            <input
              type="text"
              placeholder="Optional Site/Location Tag (e.g. Hotel 57)..."
              value={locationName}
        ...
        {/* Deployed Checkpoints & Assigned QRs Database */}
