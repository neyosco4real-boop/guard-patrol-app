...
export default function GuardScanner() {
  ...
  useEffect(() => {
    ...
    async function fetchLocations() {
      try {
        ...
        const saved = localStorage.getItem('security_locations_data');
        if (saved) setLocationsData(JSON.parse(saved));
      }
    }
  ...
  const startCameraScan = async () => {
    ...
    setStatusMessage('Starting camera scanner...');

    setTimeout(async () => {
      try {
        const scannerId = "qr-reader-strict";
        if (!scannerRef.current) {
          scannerRef.current = new Html5Qrcode(scannerId);
        }

        // Optimized settings for fast, robust mobile QR detection
        const config = { 
          fps: 20, 
        ...
        await scannerRef.current.start(
          ...
          (errorMessage) => {
            // Scanning continuously without spamming console
          }
        );
        setStatusMessage('Camera active. Point directly at QR tag.');
      } catch (err) {
        console.error('Camera start error:', err);
        setStatusMessage('Camera error. Please ensure camera permissions are allowed.');
  ...
  const handleSuccessfulScan = (codeText: string) => {
    stopCameraScan();
    console.log('Decoded QR Text:', codeText);

    let parsedLoc = '';
    let parsedCp = '';

    // 1. Try parsing JSON format generated by admin panel (e.g. {"location": "...", "checkpoint": "..."})
    try {
    ...
    // 3. Match against known database locations & checkpoints
    if (!parsedLoc) {
    ...
    // 4. Fallback if raw text scanned
    if (!parsedLoc) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!scannedLocation || !scannedCheckpoint) {
      setStatusMessage('Please scan a valid checkpoint QR code first.');
      return;
    }

    setIsSubmitting(true);
    setStatusMessage('Transmitting verified patrol log to admin live feed...');
    ...
    try {
      ...
      let finalNotes = `[Scan Time: ${currentTime.toLocaleTimeString()}] [GPS: ${gpsString}] [Geofence: 50m Active] ${notes}`;
      if (photoData) {
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 sm:p-6 flex items-center justify-center">
      <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
        ...
        <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 text-center space-y-3">
          ...
          {/* Camera Viewfinder */}
          <div className="relative overflow-hidden rounded-xl bg-black flex flex-col items-center justify-center min-h-[190px]">
            <div id="qr-reader-strict" className={`w-full ${isCameraActive ? 'block' : 'hidden'}`}></div>
            {!isCameraActive && (
              <div className="py-5 flex flex-col items-center justify-center space-y-2 px-3">
                ...
                <p className="text-xs text-slate-300 font-medium">
                  {scannedCheckpoint ? `✓ Captured: ${scannedCheckpoint}` : 'Scan checkpoint QR code to auto-fill telemetry'}
                </p>
              </div>
            )}
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          {/* Location Site (Strictly Auto-filled, Read-only for guards) */}
          <div className="space-y-1.5">
            <label className="block text-slate-400 uppercase font-semibold text-[10px] tracking-wider">Location Site (Auto-Filled by QR) *</label>
            <input
              type="text"
              readOnly
              required
              ...
              className="w-full bg-slate-950/90 border border-emerald-900/60 rounded-xl px-3.5 py-2.5 text-xs text-emerald-400 font-medium cursor-not-allowed placeholder-slate-600 shadow-inner"
            />
          </div>

          {/* Checkpoint (Strictly Auto-filled, Read-only for guards) */}
          <div className="space-y-1.5">
